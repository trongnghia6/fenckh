...
getthemTaiKhoan: async (req, res) => {
  try {
    ...
    res.render("themTk.ejs", { departmentLists: departmentLists, user: user });
  } catch (error) {
...
postthemTK : async (req, res) => {
  ...
  try {
    ...
    const query1 = `
          ...
          VALUES (?, ?, ?, ?)
      `;
    await connection.query(query1, [TenDangNhap, MaPhongBan, Quyen, isKhoa ? 1 : 0]);
...
getUpdateTK: async (req, res) => {
  try {
    ...
    // Render trang với 2 biến: value và departmentLists
    res.render("updateTK.ejs", { accountList: accountList, departmentLists: departmentLists, user: user, id: id, role: role });
  } catch (error) {
    console.error("Lỗi: ", error);
    res.status(500).send("Đã có lỗi xảy ra");
  }
